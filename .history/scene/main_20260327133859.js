import { TILE, WALL_THICKNESS, DOOR_THICKNESS, WALL_HEIGHT, FLOOR_HEIGHT, DOOR_SENSOR_DISTANCE, DOOR_CLOSE_DISTANCE, STAIR_TRIGGER_COOLDOWN_MS, ISO_X, ISO_Y, CHARACTER_FOOT_RADIUS, CHARACTER_BODY_HEIGHT, CHARACTER_HEAD_RADIUS, WORLD, FLOOR_BASE_Z, palette } from "./constants.js";
import { callQwenAPI } from "./api.js";
import { initNPCs, updateNPC, drawNPC, getWalkableBounds, NPC_TYPES, NPC_STATES } from "./npc.js";
import { createQueueManager, addToQueue, drawQueueBoard, drawRegistrationPanel, QUEUE_DEPARTMENTS, callNext } from "./queue.js";
import { createDialogSystem, openDialog, closeDialog, drawDialogBox, sendMessage, handleDialogInput } from "./dialog.js";
...
function buildDoor(spec, index) {
  ...
  if (spec.side === "top" || spec.side === "bottom") {
    ...
    return {
      id: `door-${index}`, floor: room.floor, roomKind: room.kind, ...spec,
      open: false,
...
function drawNPCCharacter(npc) {
  const top = drawCharacterBody(npc.x, npc.y, npc.floor, npc.bodyColor);
...
function drawNPCIndicator() {
  ...
  const actionText = npc.type === NPC_TYPES.NURSE ? "按 E 与护士对话" : npc.type === NPC_TYPES.DOCTOR ? "按 E 与医生对话" : npc.type === NPC_TYPES.PHARMACIST ? "按 E 与药师对话" : "";
  if (actionText) {
...
function drawHelpPanel() {
  ...
  ctx.fillText("E - 与NPC对话/挂号", panelX + 12, panelY + 54);
  ctx.fillText("1-4 - 选择科室挂号", panelX + 12, panelY + 70);
...
function drawFloorLayer(floor, activeDoor, dimmed) {
  ...
  for (const npc of npcs.filter((n) => n.floor === floor)) {
    drawables.push({ depth: characterDepth(npc.x, npc.y), draw: () => drawNPCCharacter(npc) });
  }
...
async function handleInteract() {
  ...
  if (npc.type === NPC_TYPES.NURSE) {
    ...
    dialogSystem.messages = [
      { role: "assistant", content: "您好！我是护士小李，请问您有什么不舒服的地方？", timestamp: Date.now() }
    ];
  } else if (npc.type === NPC_TYPES.DOCTOR || npc.type === NPC_TYPES.PHARMACIST) {
    openDialog(dialogSystem, npc);
    const greeting = npc.type === NPC_TYPES.DOCTOR ? "您好！我是" + npc.name + "，请坐，有什么可以帮您的？" : "您好！我是药师老张，请问需要取药吗？";
    dialogSystem.messages = [{ role: "assistant", content: greeting, timestamp: Date.now() }];
  }
}
...
async function handleDialogSend() {
  ...
  const response = await sendMessage(dialogSystem, text, dialogSystem.currentNPC, callQwenAPI);
  if (response === "error") {
...
function update(delta, nowMs) {
  ...
  if (queueManager.queues.emergency.length > 0 && Math.random() < 0.005) {
    callNext(queueManager, "emergency");
  }
}
...
function render() {
  ...
  if (dialogSystem.isOpen) {
    drawDialogBox(ctx, canvas, dialogSystem);
  }
