...
def _target_node_for_state(visit_state: str | None, assigned_department_id: str | None) -> str | None:
    ...
    if visit_state == VisitLifecycleState.WAITING_PHARMACY.value:
        return "pharmacy"
    if visit_state == VisitLifecycleState.ADMITTED.value:
        return "ward"
    return department_id
...
class FlowDecisionEngine:
    ...
    def _to_decision(self, context: FlowEngineContext) -> FlowDecision:
        ...
        if planned.action == "trigger_encounter_event":
            ...
            if event in {"request_test_payment", "pay_test", "start_exam", "finish_exam", "results_ready", "pay_medical", "plan_disposition", "choose_pharmacy", "choose_outpatient_treatment", "admit_patient", "complete_visit"}:
                ...
                if event == "admit_patient":
                    return FlowDecision(next_action="admit_patient", target_node="ward", reason=event, payload=planned.payload)
                if event == "complete_visit":
                    return FlowDecision(next_action="complete_visit", target_node=target_node or "ward", reason=event, payload=planned.payload)
