...
const backendState = {
  ...
  enabled: privateApiConfig.enabled !== false,
  connected: false,
...
function openRuntimeStatsHtmlPage() {
  if (!backendState.enabled) {
    pushStatusHint("Stats HTML requires a running backend. Frontend-only mode is enabled.");
    return;
  }
...
async function refreshIntegrationRuntime(force = false) {
  if (!backendState.enabled) return integrationState;
  const now = performance.now();
...
async function pollBackendStatuses(force = false) {
  if (!backendState.enabled) return;
  const now = performance.now();
...
if (backendState.enabled) {
  ...
  refreshIntegrationRuntime(true).catch(() => {});
  pollBackendStatuses(true).catch(() => {});
} else {
  taskBoardPresenter.syncOffline("Frontend-only mode: backend integration skipped.");
  backendState.lastError = "frontend-only mode";
}
