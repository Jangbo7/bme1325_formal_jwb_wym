const DEFAULT_PROMPT = "What would you like to ask?";

function createTalkBook({ intro, workAction, smallTalk, closing, prompt = DEFAULT_PROMPT }) {
  return {
    startNodeId: "intro",
    nodes: {
      intro: {
        type: "line",
        text: intro,
        next: "choice",
      },
      choice: {
        type: "choice",
        text: prompt,
        options: [
          { label: "Ask about work", next: "work" },
          { label: "Just chat", next: "chat" },
          { label: "Say goodbye", next: "end" },
        ],
      },
      work: {
        type: "action",
        text: workAction,
        next: "end",
      },
      chat: {
        type: "line",
        text: smallTalk,
        next: "end",
      },
      end: {
        type: "end",
        text: closing,
      },
    },
  };
}

export const roomKindLabels = {
  hall: "Main Hall",
  consultation: "Consultation Room",
  ward: "Ward",
  lab: "Laboratory",
  icu: "ICU Room",
  office: "Admin Office",
  registration: "Registration",
  triage: "Triage",
  doctor_entry: "Doctor Entry Hall",
  pharmacy_pickup: "Pharmacy Pickup",
  empty_room: "Specialty Clinic",
};

export const fixedNpcDefinitions = [
  {
    id: "hall-clerk",
    name: "Mara",
    roleLabel: "Clerk",
    floor: 1,
    roomKind: "hall",
    roomIndex: 0,
    placement: { x: 0.36, y: 0.52 },
    interactionRadius: 68,
    bodyColor: "#6d8fe8",
    headColor: "#f2d0bf",
    accentColor: "#7ac7ff",
    dialogueSetId: "hall-clerk",
  },
  {
    id: "consultation-doc",
    name: "Dr. Vale",
    roleLabel: "Doctor",
    floor: 1,
    roomKind: "consultation",
    roomIndex: 1,
    placement: { x: 0.78, y: 0.42 },
    interactionRadius: 54,
    bodyColor: "#8c7af0",
    headColor: "#f0cfbe",
    accentColor: "#c6b3ff",
    dialogueSetId: "consultation-doc",
  },
  {
    id: "consultation-doc-2",
    name: "Dr. Rowan",
    roleLabel: "Doctor",
    floor: 1,
    roomKind: "consultation",
    roomIndex: 1,
    placement: { x: 0.74, y: 0.44 },
    interactionRadius: 54,
    bodyColor: "#7e8fe8",
    headColor: "#f0cfbe",
    accentColor: "#c9d5ff",
    dialogueSetId: "consultation-doc-2",
  },
  {
    id: "triage-nurse",
    name: "Aya",
    roleLabel: "Triage Nurse",
    floor: 1,
    roomKind: "triage",
    roomIndex: 0,
    placement: { x: 0.52, y: 0.58 },
    interactionRadius: 68,
    bodyColor: "#6cc7bf",
    headColor: "#f2d0bf",
    accentColor: "#9be9e3",
    dialogueSetId: "triage-nurse",
  },
  {
    id: "ward-nurse",
    name: "Sera",
    roleLabel: "Nurse",
    floor: 1,
    roomKind: "ward",
    roomIndex: 0,
    placement: { x: 0.42, y: 0.55 },
    interactionRadius: 68,
    bodyColor: "#66bfa2",
    headColor: "#f4d1c4",
    accentColor: "#8ef0be",
    dialogueSetId: "ward-nurse",
  },
  {
    id: "icu-nurse",
    name: "Ivan",
    roleLabel: "Nurse",
    floor: 1,
    roomKind: "icu",
    roomIndex: 0,
    placement: { x: 0.58, y: 0.52 },
    interactionRadius: 68,
    bodyColor: "#d46d8e",
    headColor: "#f2ccb7",
    accentColor: "#ff9db2",
    dialogueSetId: "icu-nurse",
  },
  {
    id: "lab-tech",
    name: "Jun",
    roleLabel: "Lab Tech",
    floor: 1,
    roomKind: "lab",
    roomIndex: 0,
    placement: { x: 0.56, y: 0.48 },
    interactionRadius: 68,
    bodyColor: "#9b87f5",
    headColor: "#efcdb9",
    accentColor: "#b8a0ff",
    dialogueSetId: "lab-tech",
  },
  {
    id: "doctor-entry-clerk",
    name: "Owen",
    roleLabel: "Desk Clerk",
    floor: 1,
    roomKind: "doctor_entry",
    roomIndex: 0,
    placement: { x: 0.52, y: 0.56 },
    interactionRadius: 68,
    bodyColor: "#dca768",
    headColor: "#f0cfbe",
    accentColor: "#f3d59a",
    dialogueSetId: "doctor-entry-clerk",
  },
  {
    id: "pharmacy-clerk",
    name: "Lina",
    roleLabel: "Pharmacist",
    floor: 1,
    roomKind: "pharmacy_pickup",
    roomIndex: 0,
    placement: { x: 0.58, y: 0.52 },
    interactionRadius: 68,
    bodyColor: "#97b7e8",
    headColor: "#f4d1c4",
    accentColor: "#d8e4ff",
    dialogueSetId: "pharmacy-clerk",
  },
  {
    id: "office-admin",
    name: "Nora",
    roleLabel: "Admin",
    floor: 1,
    roomKind: "office",
    roomIndex: 0,
    placement: { x: 0.48, y: 0.52 },
    interactionRadius: 68,
    bodyColor: "#e0b65b",
    headColor: "#f3d6be",
    accentColor: "#f4c96a",
    dialogueSetId: "office-admin",
  },
  {
    id: "specialty-doc-a",
    name: "Dr. Iris",
    roleLabel: "Surgery Doctor",
    floor: 1,
    roomKind: "empty_room",
    roomIndex: 0,
    placement: { x: 0.72, y: 0.46 },
    interactionRadius: 56,
    bodyColor: "#c98672",
    headColor: "#f3d1c0",
    accentColor: "#f0c4b8",
    dialogueSetId: "specialty-doc-a",
  },
  {
    id: "specialty-doc-b",
    name: "Dr. Miko",
    roleLabel: "Specialty Doctor",
    floor: 1,
    roomKind: "empty_room",
    roomIndex: 1,
    placement: { x: 0.72, y: 0.46 },
    interactionRadius: 56,
    bodyColor: "#79a6d8",
    headColor: "#f2d0bf",
    accentColor: "#cfe3ff",
    dialogueSetId: "specialty-doc-b",
  },
];

export const fixedNpcDialogueBooks = {
  "hall-clerk": createTalkBook({
    intro: "Morning. Keep to the right if you are only passing through.",
    workAction: "Mara taps the clipboard once. \"Forms first, then the rest of the day.\"",
    smallTalk: "Some days the lobby feels like a river. Your job is to keep your balance.",
    closing: "Come back if you need directions, or a quiet place to stand for a second.",
  }),
  "consultation-doc": createTalkBook({
    intro: "The consultation rooms are at their best when the hallway stops arguing with itself.",
    workAction: "Dr. Vale points toward the next room. \"Keep your notes short. Clarity saves time.\"",
    smallTalk: "If you bring in good questions, the answers usually show up on time.",
    closing: "Take it easy. Calm hands make better charts.",
  }),
  "consultation-doc-2": createTalkBook({
    intro: "Room two is for cases that need a second pair of eyes, not a second panic.",
    workAction: "Dr. Rowan adjusts the file. \"Tell the story clearly and the chart usually follows.\"",
    smallTalk: "Most delays come from confusion, not from medicine.",
    closing: "If the flow stays clean, this room does too.",
  }),
  "triage-nurse": createTalkBook({
    intro: "Start with the headline symptom, then the details fall into place.",
    workAction: "Aya taps the intake pad. \"Temperature, timing, pain level. That usually gets us moving.\"",
    smallTalk: "The first minute of a visit decides how calm the rest of it feels.",
    closing: "When you are ready, step up and we will get the triage note started.",
  }),
  "ward-nurse": createTalkBook({
    intro: "Ward shifts are mostly small things done well.",
    workAction: "Sera checks the clipboard. \"Water, pillows, and patience. In that order.\"",
    smallTalk: "The quiet here is not empty. It is just busy in a softer way.",
    closing: "If you need me, I am usually one room away.",
  }),
  "icu-nurse": createTalkBook({
    intro: "The ICU keeps its own pace. We just keep up with it.",
    workAction: "Ivan lowers his voice. \"Short sentences. Clear notes. No wasted motion.\"",
    smallTalk: "The best part of a calm shift is noticing it after it happens.",
    closing: "Stay steady. That helps more than people think.",
  }),
  "lab-tech": createTalkBook({
    intro: "If it glows, be gentle. If it hums, label it.",
    workAction: "Jun nudges a tray of samples. \"Everything belongs somewhere. The trick is remembering where.\"",
    smallTalk: "Machines can be picky, but they are honest.",
    closing: "Bring clean notes next time. They save everyone's afternoon.",
  }),
  "doctor-entry-clerk": createTalkBook({
    intro: "Doctor entry stays smooth when everyone waits for the call.",
    workAction: "Owen checks the list. \"If your queue is green, step in. If not, give it a minute.\"",
    smallTalk: "This room is mostly timing and door control.",
    closing: "You will know it is your turn when the flow shifts in your favor.",
  }),
  "pharmacy-clerk": createTalkBook({
    intro: "Prescriptions move faster when the checkout note is complete.",
    workAction: "Lina glances at the order list. \"Payment first, pickup second, and labels always last.\"",
    smallTalk: "A tidy pharmacy feels quieter than it really is.",
    closing: "If the chart says pickup, I will be right here.",
  }),
  "office-admin": createTalkBook({
    intro: "The office runs on lists, labels, and one stubborn kettle.",
    workAction: "Nora slides a stack of papers back into order. \"A tidy desk is a kinder desk.\"",
    smallTalk: "Some days are made of paperwork. Others are made of tiny victories.",
    closing: "If you do not know where to go, the office usually does.",
  }),
  "specialty-doc-a": createTalkBook({
    intro: "This clinic handles procedural and surgery-facing cases.",
    workAction: "Dr. Iris glances at the prep board. \"A clean handoff matters more than a dramatic entrance.\"",
    smallTalk: "Specialty rooms are quieter, but the decisions are not smaller.",
    closing: "If the case needs escalation, we route it deliberately.",
  }),
  "specialty-doc-b": createTalkBook({
    intro: "This room is reserved for overflow specialty consults.",
    workAction: "Dr. Miko taps the desk. \"One room can save a queue if we keep it moving.\"",
    smallTalk: "An unused clinic is just wasted throughput.",
    closing: "When this room is open, the hallway breathes easier.",
  }),
};
