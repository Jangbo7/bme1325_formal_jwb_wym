...
def test_followup_rewording_avoids_repeating_recommendation(tmp_path, monkeypatch):
    ...
    assert RECOMMENDATION_LABEL not in reply_data["dialogue"]["assistant_message"]
    assert FOLLOWUP_REPHRASE_HINT in reply_data["dialogue"]["assistant_message"]
